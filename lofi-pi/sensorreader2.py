...
class SensorReader(threading.Thread):
	...
	def run(self):
		...
		while (self._mRun):
			...
			self._last_loop_timer = self._loop_timer
			self._loop_timer = datetime.now()
			# print(self._loop_timer - self._last_loop_timer)
			time_difference = (self._loop_timer - self._last_loop_timer).total_seconds()
			hertz = 1 / time_difference
			f_const = 1 / (hertz * 65.5)

			self._gyro_x -= self._gyro_x_cal
			self._gyro_y -= self._gyro_y_cal
			self._gyro_z -= self._gyro_z_cal

			angle_pitch += self._gyro_x * f_const
			angle_pitch += self._gyro_y * f_const

			angle_pitch += angle_roll * math.sin(self._gyro_z * f_const)
			angle_roll -= angle_pitch * math.sin(self._gyro_z * f_const)
			...
			# datetime.now() - self._loop_timer).total_seconds()
			sleep(0.001)
